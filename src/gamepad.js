...
window.addEventListener("gamepadconnected", (e) => {
  ...
  else if (localStorage.getItem("CONTROLLER-TYPE") === "wheel")
    isGamepad = false;
...
function menuControls(controller) {
  ...
  if (controller.buttons[dpadDown].pressed) {
    ...
    if (subMenuflag === 1) {
      ...
      if (urllist[Math.abs(hrefidx) % urllist.length].id === "setKey") {
        ...
        urllist[Math.abs(hrefidx) % urllist.length].id === "setDevice"
      ) {
        if (modalFlag === false) {
          urllist[Math.abs(hrefidx) % urllist.length].click();
          modalFlag = true;
        ...
        urllist[Math.abs(hrefidx) % urllist.length].id === "requiredAPI"
      ) {
        urllist[Math.abs(hrefidx) % urllist.length].click();
        modalFlag = true;
      } else if (
        urllist[Math.abs(hrefidx) % urllist.length].id === "startPoint"
      ) {
        if (modalFlag === false) {
          urllist[Math.abs(hrefidx) % urllist.length].click();
          modalFlag = true;
  ...
  } else if (controller.buttons[y].pressed) {
    // press y to allow user to use sterrignwheel
    if (
      urllist.length != 0 &&
      urllist[Math.abs(hrefidx) % urllist.length].id === "setDevice"
    ) {
...
function svDriveSteeringWheel(controller) {
  ...
  if (controller.buttons[b].pressed) {
    if (
      window.confirm(
        "Are you sure you would like to return to the menu? (use mouse to select)"
      ) === true
    ) {
      backToMenu();
    }
